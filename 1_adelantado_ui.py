...
WHERE
    ...
    AND pagos_contratados.pagos_contratados_tipo = 'RENOVACION' """
        ...
        for index, row in df.iterrows():
            ...
            if fechaAnterior != None:
                ...
                if fechaActual < fechaAnterior: #hablamos de un adelanto
                    if fechaAnterior.year == fechaActual.year:
                        if fechaAnterior.month != fechaActual.month:
                        ...
                        if fechaAnterior.month != fechaActual.month or int(dif.days) < -30: #checar esta parte para poder evaluar dias di debe ser menor a 30 dias
                            self.aux.append(row)
                             # agragar la fila correspondiente al data frame adelantados
                else:
                    pass
    ...
    def retranslateUi(self, MainWindow):
        ...
        MainWindow.setWindowTitle(_translate("MainWindow", "Adelantados"))
        ...
        dfAdelantados = pd.DataFrame(self.aux)
        #print(dfAdelantados)
        dfAdelantados.to_excel("output.xlsx")  
